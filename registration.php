<?php
  ...
  if (!empty($_POST)){
    ...
    $role=trim(@$_POST['role']);
      if (empty($role) || !in_array($role, ['autor', 'čtenář'])){
       $errors['role']='Musíte vybrat roli uživatele.';
      } 
    ...
    if (empty($errors)){
      ...
      $query=$db->prepare('INSERT INTO users (name, email, password, active,role) VALUES (:name, :email, :password, 1,:role);');
      $query->execute([
        ':name'=>$name,
        ':email'=>$email,
        ':password'=>$password,
  ...
  <form method="post">
    ...
    <div>
      <label for="role">Role:</label>
        ...
        <option value="<?php echo htmlspecialchars(@$role);?>">Vyberte roli</option>
        <option value="autor"<?php if (@$_POST['role'] == 'autor') echo ' selected' ?>>Autor</option>
        <option value="čtenář"<?php if (@$_POST['role'] == 'čtenář') echo ' selected' ?>>Čtenář</option>
        </select>
    </div>
    <div class="form-group">
